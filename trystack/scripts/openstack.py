...
def get_groups_from_server(server_vars, namegroup=True):
    ...
    for extra_group in metadata.get('ansible_host_groups', '').split(','):
        if extra_group:
...
def append_hostvars(hostvars, groups, key, server, namegroup=False):
    ssh_host = ''
    if 'interface_ip' in server and server['interface_ip'] != '':
        ssh_host = server['interface_ip']
    else:
        ssh_host = server['private_v4']
...
def get_host_groups_from_cloud(inventory):
    ...
    for server in inventory.list_hosts(**list_args):
        if 'cloud' in server and server['cloud'] != 'envvars':
            if 'interface_ip' in server or 'private_v4' in server:
